...
fn collect_files_and_folders(path: &Path) -> Vec<PathBuf> {
    ...
    let total_files = WalkDir::new(path).into_iter().filter_map(|entry| entry.ok()).count();

    println!("검색할 총 파일/폴더 개수: {}", total_files);

    let mut current = 0;
    WalkDir::new(path).into_iter().filter_map(|entry| entry.ok()).for_each(|entry| {
...
fn delete_files_and_folders(files_and_folders: &[PathBuf]) -> Vec<PathBuf> {
    ...
    files_and_folders.iter().enumerate().for_each(|(index, path)| {
        if path.is_file() {
